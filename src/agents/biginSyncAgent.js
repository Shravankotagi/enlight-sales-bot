...
async function findDeal(dealName, biginDealId, token) {
  try {
    ...
    if (customerPrefix && customerPrefix.length >= 3 && customerPrefix !== cleanSearchName) {
      const resCust = await axios.get(`${ZOHO_BIGIN_BASE}/Deals/search`, {
        headers: zohoHeaders(token),
        params: { criteria: `(Deal_Name:starts_with:${customerPrefix.substring(0, 30)})`, fields: 'id,Deal_Name,Stage' },
      });
...
async function getDealsLayout(token) {
  ...
  return { id: '1384628000000000173', name: 'Sales Pipeline', pipeline: 'Sales Standard' };
}
...
async function upsertDeal({
  ...
  if (existing) {
    try {
      ...
      console.log(`[BiginSync] Deal updated: ${dealName} → ${STAGE_MAP[stage] || 'New Inquiry'} (${existing.id})`);
      finalId = existing.id;
    } catch (err) {
...
async function pullBiginToDatabase() {
  const sb = getSupabase();
  const token = await getZohoToken();
  const results = { accountsImported: 0, contactsImported: 0, dealsImported: 0, errors: [] };

  try {
    // 0. Build Employee Name to Phone lookup
    const { data: employees } = await sb.from('employees').select('name, phone');
    ...
    const defaultRepPhone = empNameToPhoneMap.get('max') || empNameToPhoneMap.get('yash dalmia') || '918262937458';
    ...
    try {
      ...
      for (const acc of biginAccounts) {
        try {
          ...
          results.errors.push(`Account import error (${acc.Account_Name}): ${accErr.message}`);
        }
      }
    } catch (accErr) {
...
module.exports = { syncActivity, syncAllDatabaseToBigin, pullBiginToDatabase };
