...
function createTools(senderPhone, rawUserText = '') {
  ...
  const getVisitsTool = tool(
    ...
    {
      name: 'get_visits',
      description: `Retrieves customer site and field visit logs, visit outcomes (positive, neutral, negative), follow-ups due today (mode: "due_today"), overdue follow-ups (mode: "overdue"), all pending follow-ups (mode: "pending_followup"), addressed/completed follow-ups (mode: "completed" or "addressed"), visits with no follow-up required (mode: "no_followup" or "none"), all logged follow-ups (mode: "all_followups"), unvisited customers with no visits in a timeframe (mode: "not_visited" with date_range: "last_30_days"), location filtering (e.g. "Mumbai", "Pune", "Nashik"), salesperson visit leaderboard, week-over-week visit comparison, duplicate visits, visits missing location or contact person, and prospective customers who have visits logged but no orders yet (mode: "visits_no_orders").`,
      schema: z.object({
        ...
        follow_up_filter: z.string().optional().nullable().describe('Optional filter by follow-up status: "completed", "addressed", "pending", "none", "no_followup", "all_followups", "due_today", "overdue".'),
        date_range: z.string().optional().nullable().describe('Optional date filter: "today", "yesterday", "last_7_days", "this_week", "last_week", "last_30_days", "this_month", "last_month", "all".'),
        mode: z.string().optional().nullable().describe('Query mode: "list", "due_today", "overdue", "pending_followup", "completed", "addressed", "no_followup", "all_followups", "not_visited", "rep_leaderboard", "week_comparison", "duplicates", "missing_location", "missing_contact_person", "visits_no_orders". Use "completed" when user asks for addressed or completed visit follow-ups. Use "no_followup" when user asks for visits with no follow-up. Use "due_today" when user asks for visit follow-ups due today. Use "not_visited" when user asks which customers haven\'t been visited in a timeframe.'),
