...
function checkOperationalAction(lowerText) {
  ...
  return (
    ...
    /\b(complaint about|defective material|damaged material|rejected material|material rejection|rust issue|quality complaint)\b/i.test(
      lowerText,
    ) ||
...
function isQuery(text) {
  ...
  const isExplicitActionQuery =
    ...
    /\b(pending payment|who hasn|overdue|payment aging|outstanding)\b/i.test(
      lowerText,
    ) ||
...
async function getSalesThisMonth(scopeOrPhone, text = '') {
  try {
    ...
    const wonRevenue = wonDealsList.reduce(
    ...
    const wonTonnage = wonDealsList.reduce((sum, d) => {
      ...
      return (
        sum + items.reduce((iSum, it) => iSum + (Number(it.quantity) || 0), 0)
      );
    }, 0);
...
function getInquiryTonnage(inq) {
  ...
  if (ai.quantityTons && Number(ai.quantityTons) > 0)
    return Number(ai.quantityTons);
  if (ai.quantity_mt && Number(ai.quantity_mt) > 0)
    return Number(ai.quantity_mt);
...
async function getInquiriesThisMonth(scopeOrPhone, text = '') {
  try {
    ...
    let query = supabase
      .from('inquiries')
      .select(
        'id, sender_name, sender_phone, raw_text, status, source_channel, inquiry_type, media_urls, ai_extraction_json, created_at, salesperson_phone',
      )
...
async function getPendingInquiries(scopeOrPhone) {
  try {
    ...
    const title = scope.targetRepName
      ...
      : scope.isAdmin
        ? 'Company Inquiries Needing Review'
        : scope.isManager
...
async function getDealsThisWeek(scopeOrPhone) {
  try {
    ...
    const title = scope.targetRepName
      ...
      : scope.isAdmin
        ? "Company This Week's Deals"
        : scope.isManager
          ? "Team This Week's Deals"
          : "This Week's Deals";
...
async function getKRAStatus(scopeOrPhone, text = '') {
  try {
    ...
    let inqQuery = supabase
      ...
      .gte('created_at', start)
      .lte('created_at', end);
    inqQuery = applySalespersonFilter(
      inqQuery,
      scope.phones,
      'salesperson_phone',
    );
    ...
    let kraLogsQuery = supabase
      ...
      .gte('created_at', start)
      .lte('created_at', end);
    kraLogsQuery = applySalespersonFilter(
      kraLogsQuery,
      scope.phones,
      'salesperson_phone',
    );
    ...
    let visitsQuery = supabase
      ...
      .lte('visited_at', end);
    visitsQuery = applySalespersonFilter(
      visitsQuery,
      scope.phones,
      'salesperson_phone',
    );
    ...
    paymentsQuery = applySalespersonFilter(
      paymentsQuery,
      scope.phones,
      'salesperson_phone',
    );
    ...
    let recurringQuery = supabase
      ...
      .select('*')
      .eq('is_active', true);
    recurringQuery = applySalespersonFilter(
      recurringQuery,
      scope.phones,
    ...
    let complaintsQuery = supabase
      ...
      .lte('reported_at', end);
    complaintsQuery = applySalespersonFilter(
      complaintsQuery,
      scope.phones,
      'reported_by',
    );
          ...
          recurring.some(
            (r) =>
              ...
              (d.customer_name &&
                ...
                (d.customer_name
                  .toLowerCase()
                  .includes(r.customer_name.toLowerCase()) ||
                  r.customer_name
                    .toLowerCase()
    ...
    const retentionRate =
      recurring.length > 0
        ? Math.min(
            100,
            Math.round((uniqueRecurringWithOrder / recurring.length) * 100),
          )
        : 0;
    ...
    const conversionRate =
      totalDealsCount > 0 ? Math.round((wonCount / totalDealsCount) * 100) : 0;
    ...
    const totalCollected = payments.reduce(
      (s, p) => s + (Number(p.collected_amount) || 0),
      0,
    );
  ...
  try {
    ...
    const getOutcomeBreakdown = (visitsList) => {
      ...
      (visitsList || []).forEach((v) => {
        ...
        let out =
          v.outcome || (match ? match[1].toLowerCase().trim() : null);
        if (out) {
...
async function getInactiveCustomers(scopeOrPhone) {
  try {
    ...
    for (const cust of customers) {
      ...
      const lastDeal = (deals || []).find(
        (d) =>
          (d.customer_name || '').toLowerCase().includes(custName) ||
          custName.includes((d.customer_name || '').toLowerCase()),
      );
      const lastDate = lastDeal
        ? new Date(lastDeal.created_at)
      ...
      if (daysSince >= 60) {
        inactiveList.push({
          ...
          daysSince:
            daysSince === 999 ? 'No orders recorded' : `${daysSince} days ago`,
          lastDateStr:
    ...
    const title = scope.targetRepName
      ...
      : scope.isAdmin
        ? 'Company Inactive Customers (Churn Risk)'
        : scope.isManager
          ? 'Team Inactive Customers (Churn Risk)'
...
async function getReorderQueue(scopeOrPhone) {
  try {
    ...
    let taskQuery = supabase
      ...
      .limit(15);
    taskQuery = applySalespersonFilter(
      taskQuery,
      scope.phones,
      'salesperson_phone',
    );
...
async function getPaymentAging(scopeOrPhone) {
  try {
    ...
    let ptQuery = supabase
      ...
      .order('due_date', { ascending: true })
      .limit(15);
    ptQuery = applySalespersonFilter(
      ptQuery,
      scope.phones,
      'salesperson_phone',
    );
    ...
    const [{ data: ptRecords }, { data: deals }] = await Promise.all([
      ptQuery,
      dealsQuery,
    ]);
    ...
    if (ptRecords && ptRecords.length > 0) {
      ...
      const title = scope.targetRepName
        ...
        : scope.isAdmin
          ? 'Company Outstanding Payments'
          : scope.isManager
            ? 'Team Outstanding Payments'
...
async function getCustomer360(senderPhone, text, extractedName = null) {
  ...
  try {
    ...
    let dealsQuery = supabase
      .from('deals')
      .select(
        'id, stage, total_amount, customer_name, customer_phone, customer_gst, customer_address, delivery_location, payment_terms, po_number, created_at, deal_items(*)',
      )
      .ilike('customer_name', `%${customerName}%`)
      .order('created_at', { ascending: false });
    dealsQuery = applySalespersonFilter(
    ...
    const phone =
      profile?.customer_phone || deals?.[0]?.customer_phone || 'Not specified';
    const gst =
      profile?.customer_gst || deals?.[0]?.customer_gst || 'Not specified';
    const address =
      profile?.customer_address ||
      deals?.[0]?.customer_address ||
      'Not specified';
      ...
      dealsSection = deals
        .map((d, i) => {
          ...
          const items = (d.deal_items || [])
            .map(
              (it) =>
                `${it.quantity_tons || it.quantity || ''} ${it.unit || 'MT'} ${it.product_name || it.sku_text || ''}`,
            )
            .filter(Boolean)
            .join(', ');
    ...
    let paySection =
      '• No past payment records or dues currently tracked for this account.';
...
async function getReorderQueue(senderPhone) {
  try {
    ...
    let query = supabase
      .from('recurring_customers')
      .select(
        'id, customer_name, customer_phone, last_order_date, avg_order_frequency_days, notes',
      )
      .eq('is_active', true)
    ...
    const list = atRisk
      .map((a, i) => {
        const days = Math.round(
          (now - new Date(a.last_order_date).getTime()) / (1000 * 60 * 60 * 24),
        );
...
async function extractOrderFilters(text) {
  ...
  // 1. Delivery Location
  const locRegex =
    /(?:delivery\s*location|location|delivering\s*to|city|destination|at|in|to)\s*(?:is|:)?\s*([a-zA-Z0-9\s-]+?)(?:\s+(?:with|for|status|stage|above|below|more|less|on|and|$))/i;
  ...
  // 2. Stage / Status
  if (
    lower.includes('status won') ||
    lower.includes('stage won') ||
  ...
  const aboveMatch = text.match(
    /(?:above|greater\s*than|more\s*than|>|minimum|min)\s*(?:of|rs\.?|inr|₹)?\s*(\d+(?:\.\d+)?\s*(?:lakh|lacs|lac|crore|cr|k|\d{3,}))/i,
  );
  if (aboveMatch) {
    filters.min_amount = parseAmountString(aboveMatch[1]);
  }
  const belowMatch = text.match(
    /(?:below|less\s*than|under|<|maximum|max)\s*(?:of|rs\.?|inr|₹)?\s*(\d+(?:\.\d+)?\s*(?:lakh|lacs|lac|crore|cr|k|\d{3,}))/i,
  );
  if (belowMatch) {
  ...
  // 4. Quantity range
  const qtyAboveMatch = text.match(
    /(?:above|greater\s*than|more\s*than|>|minimum|min)\s*(\d+(?:\.\d+)?)\s*(?:mt|ton|tons|kgs|kg)\b/i,
  );
  if (qtyAboveMatch) {
...
`;
    ...
    if (parsed) {
      if (parsed.delivery_location && !filters.delivery_location)
        filters.delivery_location = parsed.delivery_location;
      ...
      if (parsed.min_amount != null && filters.min_amount == null)
        filters.min_amount = parsed.min_amount;
      if (parsed.max_amount != null && filters.max_amount == null)
        filters.max_amount = parsed.max_amount;
      if (parsed.min_quantity != null && filters.min_quantity == null)
        filters.min_quantity = parsed.min_quantity;
      if (parsed.max_quantity != null && filters.max_quantity == null)
        filters.max_quantity = parsed.max_quantity;
...
async function getFilteredOrders(scopeOrPhone, text = '') {
  try {
    ...
    if (filters.delivery_location) {
      query = query.or(
        `delivery_location.ilike.%${filters.delivery_location}%,customer_address.ilike.%${filters.delivery_location}%`,
      );
    }
    ...
    if (filters.delivery_location) {
      ...
      deals = deals.filter((d) => {
        ...
        return (
          dLoc.includes(locQuery) ||
          dAddr.includes(locQuery) ||
          locQuery
            .split(/\s+/)
            .some(
              (w) => w.length > 2 && (dLoc.includes(w) || dAddr.includes(w)),
            )
        );
      });
    }
    ...
    if (filters.customer_name) {
      ...
      deals = deals.filter((d) => {
        ...
        return (
          dName.includes(custQuery) ||
          custQuery.includes(dName) ||
          custQuery.split(/\s+/).some((w) => w.length > 3 && dName.includes(w))
        );
      });
    }
    ...
    if (filters.product) {
      ...
      deals = deals.filter((d) => {
        ...
        return items.some((it) => {
          ...
          return (
            combined.includes(prodQuery) ||
            prodTokens.some((tok) => combined.includes(tok))
          );
        });
      });
    }
    ...
    if (filters.min_quantity != null || filters.max_quantity != null) {
      deals = deals.filter((d) => {
        ...
        if (filters.min_quantity != null && totalQty < filters.min_quantity)
          return false;
        if (filters.max_quantity != null && totalQty > filters.max_quantity)
          return false;
        return true;
      });
    }
    ...
    if (filters.month_name || filters.year) {
      deals = deals.filter((d) => {
        ...
        if (filters.year && dDate.getFullYear() !== Number(filters.year))
          return false;
    ...
    const filterDesc = [];
    if (filters.delivery_location)
      filterDesc.push(`Location: *${filters.delivery_location}*`);
    if (filters.customer_name)
      filterDesc.push(`Customer: *${filters.customer_name}*`);
    if (filters.product) filterDesc.push(`Product: *${filters.product}*`);
    if (filters.stage) filterDesc.push(`Status: *${filters.stage}*`);
    if (filters.min_amount)
      filterDesc.push(`Min Value: *${formatINR(filters.min_amount)}*`);
    if (filters.max_amount)
      filterDesc.push(`Max Value: *${formatINR(filters.max_amount)}*`);
    if (filters.min_quantity)
      filterDesc.push(`Min Qty: *${filters.min_quantity} MT*`);
    ...
    const orderCards = displayDeals.map((d, i) => {
      ...
      const stageStr = (d.stage || 'new_inquiry').toUpperCase();
      const amountStr = d.total_amount
    ...
    const totalMatchingValue = deals.reduce(
      (s, d) => s + (Number(d.total_amount) || 0),
      0,
    );
    const totalMatchingTonnage = deals.reduce((s, d) => {
      const items = d.deal_items || [];
      return (
        s + items.reduce((iSum, it) => iSum + (Number(it.quantity) || 0), 0)
      );
    }, 0);
...
async function routeToHandler(category, text, scope, supabase, extra = {}) {
  const phone =
    typeof scope === 'object'
      ? scope.phone || (scope.phones && scope.phones[0])
      : scope;
  switch (category) {
    case 'customer_360':
...
async function handleQuery(text, senderPhone) {
  ...
  const matchedEmp = (allEmployees || []).find((emp) => {
    ...
    if (
      targetSalespersonName &&
      targetSalespersonName.toLowerCase().includes(empNameLower)
    )
      return true;
  ...
  if (
    ...
    lower.includes('report card') ||
    (lower.includes('report') && lower.includes('kra'))
  ) {
    return await generateFullKRAReport(
      effectiveScope,
  ...
  if (
    ...
    lower.includes('week deals')
  ) {
    return await getDealsThisWeek(effectiveScope);
  }
  // Pending deals
  if (
    lower.includes('pending deal') ||
    lower.includes('open deal') ||
