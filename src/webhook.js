  ...
  try {
    ...
    if (body.object && body.entry && body.entry[0] && body.entry[0].changes && body.entry[0].changes[0]) {
      ...
      if (value.messages && value.messages[0]) {
        ...
        // --- CHECK ACTIVE REJECTION & PENDING STATE FLOWS (multi-turn logic) ---
        const isMedia = messageType === 'image' || messageType === 'document' || (media_urls && media_urls.length > 0);
        if (isMedia) {
          await saveActiveSession(senderPhone, 'Unknown', 'general');
        } else {
          const { handlePendingSessionState } = require('./core/pendingStateManager');
          const pendingReply = await handlePendingSessionState(raw_text, senderPhone);
          if (pendingReply) {
            await sendTextMessage(senderPhone, pendingReply);
            return;
          }
        }
        ...
        if (deal && extraction && extraction.line_items && extraction.line_items.length > 0) {
          ...
          const code = deal.deal_number ? deal.deal_number.replace(/^#?(?:DEAL|INQ)-?/i, '') : deal.id.substring(0, 6).toUpperCase();
          replyMessage = `${status} - Inquiry INQ-${code}\n\n` +
